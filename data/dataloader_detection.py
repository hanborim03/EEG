...
def computeSliceMatrix(
    ...
    Args:
        ...
        edf_fn: file name of the original EDF file (used to get seizure times)
    ...
    # Check if the .h5 file exists before proceeding
    if not os.path.exists(h5_fn):
        print(f"File not found: {h5_fn}, skipping this file.")
    ...
    try:
        ...
        print(f"Error processing {h5_fn}: {str(e)}")
        return None, None  
        ...
        try:
            ...
            if self.preproc_dir is None:
                ...
                if not os.path.exists(resample_sig_dir):
                    print(f"Resampled signal file not found: {resample_sig_dir}")
                    return None
                ...
                h5_file = os.path.join(self.preproc_dir, h5_fn)
                if not os.path.exists(h5_file):
                    print(f"Preprocessed file not found: {h5_file}")
                    return None
                with h5py.File(h5_file, 'r') as hf:
                    eeg_clip = hf['clip'][()]

            if eeg_clip is None or len(eeg_clip) == 0:
                print(f"Empty EEG clip for {h5_fn}")
                return None
            ...
            if self.graph_type == 'individual':
                ...
                if np.any(np.isnan(curr_support)):
                    print(f"NaN found in individual supports for {h5_fn}")
                    return None
...
def custom_collate(batch):
    ...
    # 모든 샘플의 첫 번째 차원을 최대 길이로 패딩
    max_len = max(x[0].shape[0] for x in batch)
    batch = [(F.pad(x[0], (0, 0, 0, 0, 0, max_len - x[0].shape[0])), *x[1:]) for x in batch]
...
def load_dataset_detection(
    ...
    for split in ['train', 'dev', 'test']:
        ...
        try:
            ...
            logger.info(f"Loaded {len(dataset)} samples for {split} set")
        except Exception as e:
