...
class DCRNNModel_classification(nn.Module):
    ...
    def get_features(self, input_seq, seq_lengths, supports):
        activations = {}  # 활성화를 저장할 딕셔너리 초기화
        batch_size, max_seq_len = input_seq.shape[0], input_seq.shape[1]
        ...
        # 인코더 활성화 저장
        encoder_output, final_hidden = self.encoder(input_seq, init_hidden_state, supports)
        activations['encoder'] = encoder_output  # 인코더 출력 저장
        ...
        # 마지막 출력 형태 조정
        last_out = last_out.view(batch_size, self.num_nodes, self.rnn_units)
        activations['decoder'] = last_out  # 디코더 출력 저장
    
        # FC 레이어 활성화 저장
        logits = self.fc(self.relu(last_out))
        activations['fc'] = logits  # FC 레이어 출력 저장

        return activations  # 활성화를 포함한 딕셔너리 반환
